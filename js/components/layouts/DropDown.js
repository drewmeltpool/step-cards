...
export class DropDownBootstrap {
    constructor(title, ...items) {
        this.items = items.map(item => {
            return new Element().tag('li').options({className: 'dropdown__info', textContent: item})
        })
        ...
        this.dropdown = new Element()
            ...
            .children(
                new Element().tag('button').options({
                    type: 'button',
                    className: 'button btn--option',
                    dataset: {name: 'bsToggle', value: 'dropdown'},
                    textContent: title,
                }).eventListener('click', (e) => {
                    const dropDownMenu = e.target.closest('.dropdown').querySelector('.dropdown__menu')
